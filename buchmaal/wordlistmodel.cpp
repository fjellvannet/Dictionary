  ...
  if(sortLanguage() == unsorted || sortLanguage() != a_sortLanguage) {
    ...
    switch(sortLanguage()) {
      case Deutsch:
        ...
        setQuery("SELECT DISTINCT DE,DE_type,DE_Sect FROM WORDLIST ORDER BY DE_Sort,DE_type");
        break;
      case Bokmaal:
        qDebug().noquote() << "sorted in Bokmaal";
        setQuery("SELECT DISTINCT NO,NO_type,NO_Sect FROM WORDLIST ORDER BY NO_Sort,NO_type");
        break;
      default:
        ;//feilmelding
    }
